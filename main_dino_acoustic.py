...
def get_args_parser():
    ...
    # Misc
    parser.add_argument('--data_path', default='/scratch/disk5/ahmet/acoustic_images', type=str,
        help='Please specify path to the ImageNet training data.')
    parser.add_argument('--output_dir', default="/scratch/disk5/ahmet/dino_output/100ep_acoustics_8w_Random_Bg_9_TrainSurveys_NEAREST_ResNet18Modified_128NONNormalized_30Warmup_1e5-0005LR_8192Out_FixAug_256Batch_10Iters",
    ...
    # Acoustic Data Arguments
    parser.add_argument("--train_iters", default=10, type=int, help='Number of iterations for train data loader.')
    parser.add_argument("--train_years", default=[2007, 2008, 2009, 2010, 2011, 2013, 2014, 2015, 2016], type=int, help='Survey year for training.') #
    parser.add_argument("--window_size", default=[8, 8], type=int, help='Window size for acoustic data.')
    parser.add_argument("--sampling_strategy_acoustic", default='Random_Bg',
...
def train_dino(args):
    ...
    # Defining Dataset and Dataloader for Acoustic Data
    data_object = DataZarr(frequencies=[18, 38, 120, 200], patch_size=args.window_size, partition_train='selected surveys',
                           train_surveys=args.train_years,
                           validation_surveys=[2017], partition_predict=[2017],
                           evaluation_surveys=[2017], save_prediction_surveys=False, eval_mode='all',
                           sampling_strategy=args.sampling_strategy_acoustic, patch_overlap=20)
    data_loader = define_data_loaders(data_object, batch_size=args.batch_size_per_gpu, iterations=args.train_iters
                                      , test_iter=2, patch_size=args.window_size, meta_channels=[], num_workers=args.num_workers)[0] # train dataloader
   ...
   #     drop_last=True)
    ...
    # otherwise, we check if the architecture is in torchvision models
        ...
        student = modified_resnet()
        teacher = modified_resnet()
        embed_dim = student.fc.weight.shape[1]
    elif args.arch == 'modified_resnet18_64':
        student = modified_resnet_resnet18_64()
        teacher = modified_resnet_resnet18_64()
        embed_dim = student.fc.weight.shape[1]
    elif args.arch == 'modified_resnet18_128':
        student = modified_resnet_resnet18_128()
        teacher = modified_resnet_resnet18_128()
        embed_dim = student.fc.weight.shape[1]
    else:
...
def train_one_epoch(student, teacher, teacher_without_ddp, dino_loss, data_loader,
    ...
    for it, (images_hepsi) in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        images_a = DataAugmentationDINO(args.global_crops_scale, args.local_crops_scale,
                                        args.local_crops_number)(images_hepsi['data'].to(torch.float32))
        # move images to gpu
        images = [im.cuda(non_blocking=True) for im in images_a]

        # Printing the original image, and local and global crops
        #plt.imshow(images_hepsi['data'][0, 2, :, :])
