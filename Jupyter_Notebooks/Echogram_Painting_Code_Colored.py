...
colors_cluster4 = ['red', 'tab:blue', 'tab:green', 'tab:orange', 'tab:purple', 'tab:cyan',
                   'tab:brown']
cmap_for_Sv = mcolors.ListedColormap(colors_cluster4)

colors_cluster5 = ['tab:purple', 'tab:blue', 'tab:brown', 'tab:orange', 'tab:green', 'tab:cyan',
                   'red']
cmap_for_SSL = mcolors.ListedColormap(colors_cluster5)

colors_cluster6 = ['black', 'red', 'red']
...
def visualize_data(ping_start_value, ping_end_value, range_start, range_end, num_clusters,
                   Sv_ayarlanmis_data, features_echogram, cmap_for_Sv, cmap_for_SSL, colorbars=False, plot_Sv_clusters=True,
                   savefig=False):
    ...
    knn = KNeighborsClassifier(n_neighbors=15, metric='euclidean').fit(features_echogram.iloc[:100, 11:139],
                                                                       features_echogram.iloc[:100, 1])
    echogram_classes = knn.predict(features_echogram.iloc[:, 11:139])
    final_data = pd.concat \
        ([pd.DataFrame(kmeans_classes), pd.DataFrame(echogram_classes), pd.DataFrame(kmeans_Sv_original_classes),
          features_echogram.iloc[:, 3:5]], axis=1)
    ...
    ax[2 + plot_Sv_clusters].text(-0.07, 0.5, '(d)', fontsize=12, va='center', ha='center',
                                  transform=ax[2 + plot_Sv_clusters].transAxes)
    if colorbars:
...
visualize_data(ping_start_value, ping_end_value,
               ...
               features_echogram, cmap_for_Sv, cmap_for_SSL, colorbars=True, plot_Sv_clusters=True, savefig=False)
