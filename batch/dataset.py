...
from utils_unet.np import getGrid, new_get_crop_2d, new_get_crop_3d, patch_coord_to_data_coord
...
class Dataset():
    ...
    def __getitem__(self, index):
        ...
        year = data_reader.year


        # Defining the type of patch by using the sampler name and fish type (if applicable)
        patch_class = sampler.__class__.__name__
        try:
            patch_class = patch_class + '_' + str(sampler.fish_type)
        ...
        if len(self.meta_channels) == 0:
            return {'data': data, 'labels': labels, 'center_coordinates': np.array(center_location),
                    'patch_class': patch_class, 'year': year}
        else:
...
class DatasetTest():
    ...
    def __getitem__(self, index):
        ...
        i = np.random.rand()
        sampler = self.samplers[np.where(i < self.sampler_probs)[0][0]]
        center_location, data_reader, label, n_other, n_sandeel, n_bottom, Sv_200_mean, n_total = sampler.get_sample()
        year = data_reader.year
        ...
        if len(self.meta_channels) == 0:
            return {'data': data, 'labels': labels, 'center_coordinates': np.array(center_location),
                    'patch_class': label, 'year': year, 'n_other': n_other, 'n_sandeel': n_sandeel,
                    'n_bottom': n_bottom, 'Sv_200_mean': Sv_200_mean, 'n_total': n_total}
        else:
            return {'data': np.concatenate((data, meta), axis=0), 'labels': labels,
                    'center_coordinates': np.array(center_location)}
